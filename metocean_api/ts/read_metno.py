...
def NORAC_ts(ts: TimeSeries, save_csv=False, save_nc=False, use_cache=False):
    ...
    tempfiles = __tempfile_dir(ts.product, ts.lon, ts.lat, dates)
            ...
            with xr.open_dataset(url) as dataset:
                ...
                for var_name in ["wind_speed", "wind_direction"]:
                    if var_name in dataset:
                        dataset[var_name].encoding["_FillValue"] = dataset[
                            var_name
                        ].attrs["fill_value"]
...
def __tempfile_dir(product, lon, lat, date_list):
    tempfiles = []
    # Create directory
    dir_name = "cache"
    try:
    ...
    for i in range(len(date_list)):
        tempfiles[i] = str(Path(dir_name+"/"+product+"_"+"lon"+str(lon)+"lat"+str(lat)+"_"+date_list.strftime('%Y%m%d')[i]+".nc"))
